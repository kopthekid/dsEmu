...
def _ensure_emscripten_available(command: str):
  if shutil.which(command):
    return True

  output.error(f'Missing required command: {command}')
  output.info('Install and activate Emscripten, then retry.')
  output.info('Example setup: git clone https://github.com/emscripten-core/emsdk.git && cd emsdk && ./emsdk install latest && ./emsdk activate latest && source ./emsdk_env.sh')
  return False
...
def wasm_build_cmake():
  ...
  if not _ensure_emscripten_available('emcmake'):
    return False
...
def wasm_build():
  ...
  if not _ensure_emscripten_available('emmake'):
    return False
